...
#if(IIC_CONFIG==1)
	#define i2c_write   ExtIIC_SendAddr7
	#define i2c_read    ExtIIC_ReceiveAddr7
#else
	#define i2c_write   Write_Len
	#define i2c_read    Read_Len
#endif
...
u8 mpu_dmp_init(void)
{
	...
	{	 
		...
		if(res)return 7;   
		res=run_self_test();		//自检(若陀螺仪不水平，则自检失败)
		if(res)return 8;    
